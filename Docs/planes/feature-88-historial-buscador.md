...
- ⬜ Mover el bloque `TextField` de búsqueda + su `toolbar` (Filtros/Limpiar) para renderizarse
  **después** del bloque `destacados` ("Lo último") y **antes** del `segmented`.
- ⬜ Mantener búsqueda en vivo + filtros del modal + estados de carga/error donde tienen sentido.
- ⬜ Test `HistoryScreen.test.tsx`: el buscador aparece tras "Lo último" y antes del toggle; el
