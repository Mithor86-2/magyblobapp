...
- [ ] **`pnpm audit --audit-level=high`** como paso (informativo al principio) en el job `gate`.
      ...
      a bloqueante.
      _Nota (2026-07-08):_ saneadas **las 7** vulnerabilidades transitivas (1 high + 5 moderate + 1 low)
      con `overrides` en `pnpm-workspace.yaml` — ver
      [planes/deps-vulnerabilidades.md](planes/deps-vulnerabilidades.md). La última pendiente
      (`uuid` v3/v5/v6, buffer bounds, vía `@expo/cli › xcode`) se cerró forzando `uuid ≥11.1.1`
      (resuelto 14.0.1): el riesgo de prebuild era infundado, `xcode` solo llama a `uuid.v4()` sin
      buffer y es compatible. `pnpm audit`: 0 vulnerabilidades.
