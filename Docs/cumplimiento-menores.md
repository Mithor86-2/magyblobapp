...
| C-17 | Verificación de titularidad del email (OTP/SMTP, US-93) | **Refuerzo (no desviación) de C-1/C-10:** al crear la cuenta, con SMTP configurado, el backend envía un **código OTP de 6 dígitos** al email del adulto y **no emite sesión** hasta validarlo, comprobando que el email es real y suyo (materializa el "doble opt-in" que C-16/US-92 dejó como mejora futura). El correo viaja por un **servidor SMTP** (contacto server-side con un tercero) pero transporta **solo el email del adulto y un código numérico** —nunca datos del menor ni identificadores— y **no** añade ningún SDK de terceros a la app. El código se guarda **hasheado (bcrypt)**, nunca en claro ni en logs, con caducidad e intentos limitados; el secreto SMTP vive en variables de entorno, nunca en BD ni en el repo. **Sin SMTP configurado la verificación se omite** (cuenta verificada al alta) y **no sale nada de la máquina**, preservando el arranque reproducible (C-6/US-06). Ver [US-93](historias-usuario/epic-f-plataforma.md#us-93)                                                                  | Refuerzo / OK sin SMTP          |
