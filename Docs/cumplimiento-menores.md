...
| C-6  | Parental gate (zona de adultos)                    | Configuración y gestión de perfiles tras una puerta parental (cliente `ParentalGate`; **además reforzada server-side en el alta**, US-92: el alta exige superar un reto firmado, no basta con marcar la casilla); UI infantil sin enlaces externos ni compras                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Diseño                          |
...
| C-15 | Portada de imagen con Gemini (US-59)               | **Desviación asumida (TFM), análoga a C-5:** generar la **portada ilustrada** de un cuento/actividad envía a un tercero (Google Gemini/Imagen) el **tema/estilo/título** del contenido. El título suele contener el nombre del niño, así que se **redacta el nombre** del título antes de salir (`redactarNombre`, palabra completa, case-insensitive) → solo viajan tema/estilo/título sin PII del menor ni identificadores. **Rompe C-5** como salida a un tercero e **incompatible con Apple Kids**. Mitigaciones: (a) **sin `GEMINI_API_KEY` no se genera nada** y la app usa solo el **respaldo local empaquetado** (privacidad por diseño, sin red); (b) la generación es **best-effort** (si falla, campo `NULL` y respaldo local) — no es funcionalidad crítica; (c) la clave vive solo en env del backend, nunca en el repo ni en BD; (d) el free tier de Gemini puede registrar/entrenar con el prompt — para datos reales de menores haría falta DPA y no-entrenamiento. Ver [US-59](historias-usuario/epic-b-cuentos.md#us-59)                                                                                          | Desviación (TFM) / OK sin key   |
| C-16 | Endurecimiento del API público (US-92)             | **Refuerzo (no desviación):** la superficie pública del backend se endurece frente al abuso automatizado sin añadir terceros. (a) **Rate limiting** (`@fastify/rate-limit`) en `POST /guardians`, `/guardians/login` y `/guardians/refresh` (429 al superar el umbral), con la IP real del cliente tras el proxy (`trustProxy`) → frena fuerza bruta, credential stuffing y alta masiva. (b) **Puerta parental server-side** en el alta: reto aritmético firmado con HMAC (secreto JWT) y caducidad, sin estado en BD y **sin red externa** → materializa C-6 en el backend, **no afecta a C-2/C-5**. (c) **Cabeceras de seguridad** (`@fastify/helmet`) y **CORS** con allowlist. Limitación asumida: el reto no es un CAPTCHA fuerte y la **verificación de titularidad del email** (doble opt-in) queda como mejora futura (necesitaría un proveedor SMTP = tercero). Ver [US-92](historias-usuario/epic-f-plataforma.md#us-92)                                                                                                                                                                                                  | Refuerzo / OK                   |
| C-17 | Verificación de titularidad del email (OTP, US-93) | **Refuerzo (no desviación) de C-1/C-10:** al crear la cuenta, con proveedor de email configurado, el backend envía un **código OTP de 6 dígitos** al email del adulto y **no emite sesión** hasta validarlo, comprobando que el email es real y suyo (materializa el "doble opt-in" que C-16/US-92 dejó como mejora futura). El correo se entrega server-side por un tercero —**Brevo vía API HTTP** en producción (Render **bloquea el egress SMTP**), o SMTP en hosts que lo permitan— pero transporta **solo el email del adulto y un código numérico** —nunca datos del menor ni identificadores— y **no** añade ningún SDK de terceros a la app. El código se guarda **hasheado (bcrypt)**, nunca en claro ni en logs, con caducidad e intentos limitados; las credenciales del proveedor (`BREVO_API_KEY`/`EMAIL_FROM` o `SMTP_*`) viven en variables de entorno, nunca en BD ni en el repo. **Sin proveedor configurado la verificación se omite** (cuenta verificada al alta) y **no sale nada de la máquina**, preservando el arranque reproducible (C-6/US-06). Ver [US-93](historias-usuario/epic-f-plataforma.md#us-93) | Refuerzo / OK sin proveedor     |
