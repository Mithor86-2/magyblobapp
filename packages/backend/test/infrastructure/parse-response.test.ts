 ...
 */
describe('parseStory', () => {
  it('recorta y estampa el proveedor y el prompt cuando el cuento es válido', () => {
  ...
  it('lanza si falta el título', () => {
    expect(() => parseStory({ titulo: '   ', cuerpo: 'Cuerpo.' }, 'Ollama', 'local', 'p')).toThrow(
      /sin título o sin cuerpo/,
    );
  });

  it('lanza si falta el cuerpo', () => {
    expect(() => parseStory({ titulo: 'Título' }, 'Cloud', 'cloud', 'p')).toThrow(
      /sin título o sin cuerpo/,
    );
  });
  ...
  it('trata tipos no-string como vacío (lanza)', () => {
    expect(() =>
      parseStory({ titulo: 42 as unknown, cuerpo: null as unknown }, 'Ollama', 'local', 'p'),
...
describe('parseActivities', () => {
  ...
  it('acepta actividades válidas, recorta y estampa el proveedor', () => {
    ...
    // US-61: el prompt usado se estampa en todas las actividades del lote.
    expect(acts.every((a) => a.prompt === 'PROMPT')).toBe(true);
  ...
  it('lanza si `actividades` no es un array', () => {
    expect(() =>
      parseActivities({ actividades: 'nope' as unknown }, 3, 'Ollama', 'local', 'PROMPT'),
  ...
  it('lanza si no queda ninguna actividad válida tras sanear', () => {
    expect(() =>
      parseActivities(
        { actividades: [{ ...ok, categoria: 'inexistente' }] },
        3,
        'Cloud',
        'cloud',
        'PROMPT',
