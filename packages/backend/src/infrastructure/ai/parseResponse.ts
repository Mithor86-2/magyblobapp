 ...
 * `fuente` solo se usa para el mensaje de error (qué proveedor falló).
 */
...
export function parseActivities(
  ...
  const actividades = crudas.map(parseActividad).filter((a): a is GeneratedActivity => a !== null);
...
function parseActividad(raw: unknown): GeneratedActivity | null {
